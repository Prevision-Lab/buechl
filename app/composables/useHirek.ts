    ...
    try {
      ...
      const processHir = (hir: any) => {
        ...
        const galeriaKepek = hir.galeria?.map((item: any) => {
          ...
          return {
            src: `${config.public.directusUrl}/assets/${file.id}?access_token=${config.public.directusToken}`,
        ...
        return {
          ...
          kepUrl: hir.kiemelt_kep
            ? `${config.public.directusUrl}/assets/${hir.kiemelt_kep}?access_token=${config.public.directusToken}`
            : null,
          videoUrl: hir.video
            ? `${config.public.directusUrl}/assets/${hir.video}?access_token=${config.public.directusToken}`
            : null,
          galeriaKepek
        }
      }
